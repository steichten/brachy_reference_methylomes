#!/bin/bash
set -u
# Bisulfite sequence analysis pipeline for GR paper
# SRE
# Updated 20-9-2016
###################
...
echo "USAGE: wgbs_se_pipelinev0.2.sh <-se_epi, or -pese> <in fastq R1> <in fastq R2 (if PE)> <path to bismark genome folder> <fileID for output files>"
exit 1
fi
...
bismark -n 2 -l 20 ../../$genome_path  ${fq_file1%%.fastq*}_val_1.fq.gz_unmapped_reads_1.fq 2>&1 | tee -a ../${fileID}_logs_${dow}.log

#part 3 - SE directional pbat on unmapped R2
bismark -n 2 -l 20 --pbat ../../$genome_path  ${fq_file2%%.fastq*}_val_2.fq.gz_unmapped_reads_2.fq 2>&1 | tee -a ../${fileID}_logs_${dow}.log
...
map_ef2=$(grep 'Mapping efficiency:' 4_bismark_alignment/${fq_file1%%.fastq*}_val_1.fq.gz_unmapped_reads_1.fq*_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t' | cut -d'%' -f1)
unique_aln2=$(grep 'Number of alignments with a unique best hit from the different alignments:' 4_bismark_alignment/${fq_file1%%.fastq*}_val_1.fq.gz_unmapped_reads_1.fq*_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t')
no_aln2=$(grep 'Sequences with no alignments under any condition:' 4_bismark_alignment/${fq_file1%%.fastq*}_val_1.fq.gz_unmapped_reads_1.fq*_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t')
multi_aln2=$(grep 'Sequences did not map uniquely:' 4_bismark_alignment/${fq_file1%%.fastq*}_val_1.fq.gz_unmapped_reads_1.fq*_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t')
cpg_per2=$(grep 'C methylated in CpG context:' 4_bismark_alignment/${fq_file1%%.fastq*}_val_1.fq.gz_unmapped_reads_1.fq*_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t' | cut -d'%' -f1)
chg_per2=$(grep 'C methylated in CHG context:' 4_bismark_alignment/${fq_file1%%.fastq*}_val_1.fq.gz_unmapped_reads_1.fq*_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t' | cut -d'%' -f1)
chh_per2=$(grep 'C methylated in CHH context:' 4_bismark_alignment/${fq_file1%%.fastq*}_val_1.fq.gz_unmapped_reads_1.fq*_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t' | cut -d'%' -f1)

map_ef3=$(grep 'Mapping efficiency:' 4_bismark_alignment/${fq_file2%%.fastq*}_val_2.fq.gz_unmapped_reads_2.fq_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t' | cut -d'%' -f1)
unique_aln3=$(grep 'Number of alignments with a unique best hit from the different alignments:' 4_bismark_alignment/${fq_file2%%.fastq*}_val_2.fq.gz_unmapped_reads_2.fq_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t')
no_aln3=$(grep 'Sequences with no alignments under any condition:' 4_bismark_alignment/${fq_file2%%.fastq*}_val_2.fq.gz_unmapped_reads_2.fq_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t')
multi_aln3=$(grep 'Sequences did not map uniquely:' 4_bismark_alignment/${fq_file2%%.fastq*}_val_2.fq.gz_unmapped_reads_2.fq_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t')
cpg_per3=$(grep 'C methylated in CpG context:' 4_bismark_alignment/${fq_file2%%.fastq*}_val_2.fq.gz_unmapped_reads_2.fq_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t' | cut -d'%' -f1)
chg_per3=$(grep 'C methylated in CHG context:' 4_bismark_alignment/${fq_file2%%.fastq*}_val_2.fq.gz_unmapped_reads_2.fq_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t' | cut -d'%' -f1)
chh_per3=$(grep 'C methylated in CHH context:' 4_bismark_alignment/${fq_file2%%.fastq*}_val_2.fq.gz_unmapped_reads_2.fq_bismark_SE_report.txt  | cut -d: -f2 | tr -d '\t' | cut -d'%' -f1)
